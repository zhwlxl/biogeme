...
bioArithSum::bioArithSum(bioExpressionRepository* rep,
			 ...
			 patULong w,
			 patError*& err) 
  : bioArithIterator(rep, par,left,it,err),bhhh(NULL),weightId(w),accessToFirstRow(patBadId),theScale(0.0),scaleDerivatives(patFALSE),theRowIter(NULL),theMetaIter(NULL) {
...
patReal bioArithSum::getValue(patBoolean prepareGradient, patULong currentLap, patError*& err)  {
  ...
  if(currentLap > lastComputedLap || currentLap == patLapForceCompute){
    ...
    if (theIteratorType == ROW) {
      ...
      patULong nIter(0) ;
      for (theRowIter->first() ; !theRowIter->isDone() ; theRowIter->next()) {
        ++nIter ;
      ...
      for (theMetaIter->first() ; !theMetaIter->isDone() ; theMetaIter->next()) {
	...
	if (accessToFirstRow == patBadId) {
	  ...
	  if (accessToFirstRow) {
	    child->setVariables(theMetaIter->getFirstRow()) ;
	  }
	}
        patReal expression = child->getValue(patFALSE, patLapForceCompute, err) ;
        if (err != NULL) {
...
void bioArithSum::setScale(patError*& err) {
  ...
  if (theIteratorType == ROW) {
    ...
    for (theRowIter->first() ;
	 ...
	 theRowIter->next()) {
      ...
      if (err != NULL) {
	patULong theRow = theRowIter->getCurrentRow() ;
	stringstream str ;
...
patIterator<bioFunctionAndDerivatives*>* bioArithSum::getIteratorForStochasticGradient() {
  return this ;
}
