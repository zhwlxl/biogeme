...
bioMinimizationProblem::bioMinimizationProblem(trFunction* f,trBounds b, trParameters p):
  theFunction(f), theBounds(b) , theTrParameters(p), theOptimizationResults(NULL) {
...
bioOptimizationResults* bioMinimizationProblem::getOptimizationResults(patVariables x,
								      patBoolean secondDerivatives,
								      patError*& err) {

  if (theOptimizationResults != NULL) {
    DELETE_PTR(theOptimizationResults) ;
  }
  theOptimizationResults = new bioOptimizationResults(secondDerivatives,nVariables()) ;

  theOptimizationResults->solution = x ;
  if (secondDerivatives) {
    theOptimizationResults->prepareMemoryForMatrices(theTrParameters,err) ;
    if (err != NULL) {
      WARNING(err->describe()) ;
      return NULL ;
    }
  ...
  patBoolean analyticalHessian = bioParameters::the()->getValueInt("calculateAnalyticalHessian",err) ;
  if (err != NULL) {
    WARNING(err->describe()) ;
    return NULL ;
  }
  
  patBoolean findiffHessian = bioParameters::the()->getValueInt("calculateFinDiffHessian",err) ;
  if (err != NULL) {
    WARNING(err->describe()) ;
    return NULL ;
  }
  ...
  if (secondDerivatives && analyticalHessian && theFunction->isHessianAvailable()) {
    
    theOptimizationResults->objective =
      theFunction->computeFunctionAndDerivatives(&x,
						 &theOptimizationResults->gradient,
						 theOptimizationResults->hessian,
						 &success,
						 err) ;
    if (err != NULL) {
      WARNING(err->describe());
  ...
  if (!success) {
    WARNING("Unsuccessful computation of the function and its derivatives.") ;
    return NULL ;
  }

  if (theOptimizationResults->bhhh != NULL) {
    theFunction->computeCheapHessian(theOptimizationResults->bhhh,err) ;
    if (err != NULL) {
      WARNING(err->describe());
      return NULL;
    }
  ...
  if (secondDerivatives && !analyticalHessian && findiffHessian) {
    theFunction->computeFinDiffHessian(&x,theOptimizationResults->hessian,&success,err) ;
    if (err != NULL) {
      WARNING(err->describe());
      return NULL;
    }
    if (!success) {
      WARNING("Unsuccessful computation of the hessian via finite differences.") ;
      return NULL ;
    }
  }
  e.setTimeOfDay() ;
  patTimeInterval ti0(b,e) ;
  GENERAL_MESSAGE("Calculation of the final derivatives: " << ti0) ;
  return theOptimizationResults ;
}
